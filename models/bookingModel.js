...
//    const sqlstring = "SELECT * FROM bookings WHERE phone_number = $1 AND status = 'pending' order by id desc limit(1)";
...
async function createBooking(phoneNumber, context) {
    ...
    let bookingRef = await getBookingByPhoneNumber(phoneNumber);
    client.release();
    return JSON.stringify(bookingRef.booking_reference) // Return the booking reference of the newly created booking
}
 ...
 */
async function updateInfoForBooking(phoneNumber, message) {
    const pool = getPool();
    const client = await pool.connect();
    const sqlstring = "update bookings set extra_info = (extra_info || '; ' || $1)";
    await client.query(sqlstring, [message]);
    client.release();
}
